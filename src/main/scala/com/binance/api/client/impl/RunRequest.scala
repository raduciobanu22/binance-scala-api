    ...
    call.enqueue(new Callback[ResponseBody]() {
      override def onResponse(call: Call[ResponseBody], response: Response[ResponseBody]): Unit =
        if (response.isSuccessful) parser.decode[T](response.body.string()) match {
          ...
          if (response.code == 504) { // HTTP 504 return code is used when the API successfully sent the message but not get a response within the timeout period.
          ...
          Try(parser.decode[BinanceApiError](errorBodyString)) match {
            case Success(Right(parsedError)) =>
              promise.failure(BinanceApiExceptionError(parsedError))
            case _ =>
              promise.failure(
                BinanceApiExceptionMsg(
      ...
      override def onFailure(call: Call[ResponseBody], t: Throwable): Unit =
        promise.failure(t)
    })
